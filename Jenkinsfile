        ...
        when {
         ...
         steps {
           node(label: 'docker') {
             script {
               try {
                 sh '''docker pull eeacms/clms-backend:5.34; docker run -d --name="$BUILD_TAG-clms-backend" -e SITE="Plone" -e PROFILES="clms.addon:default clms.downloadtool:default clms.statstool:default clms.types:default eea.volto.policy:default collective.volto.cookieconsent:default collective.volto.dropdownmenu:default" eeacms/clms-backend:5.34'''
