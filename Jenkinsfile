        ...
        when {
         ...
         steps {
           node(label: 'docker') {
             script {
               try {
                 sh '''docker pull eeacms/clms-backend:5.32; docker run -d --name="$BUILD_TAG-clms-backend" -e SITE="Plone" -e PROFILES="clms.addon:default clms.downloadtool:default clms.statstool:default clms.types:default" eeacms/clms-backend:5.32'''
